package com.gamedesign.pacman.collision;

import com.almasb.ents.Entity;
import com.almasb.fxgl.app.FXGL;
import com.almasb.fxgl.physics.CollisionHandler;
import com.gamedesign.pacman.EntityFactory;
import com.gamedesign.pacman.PacmanApp;
import com.gamedesign.pacman.type.EntityType;
import javafx.util.Duration;

/**
 * Created by 1072524 on 3/6/2017.
 */
public class PlayerPointBoostHandler extends CollisionHandler
{
    public PlayerPointBoostHandler(EntityType player, EntityType pointBoost) {
        super(player, pointBoost);
    }

    @Override
    protected void onCollisionBegin(Entity player, Entity pointBoost) {
        PacmanApp app = (PacmanApp) FXGL.getApp();
        app.setScore(app.getScore() + 500);
        app.getGameWorld().addEntity(EntityFactory.newPopUp("+500", Duration.millis(500)));

        app.getAudioPlayer().playSound("pacman_eatfruit.wav");

        pointBoost.removeFromWorld();
    }
}
